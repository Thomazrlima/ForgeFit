Feature: Acompanhamento de Bioimpedância
  ...
  Scenario: Registro de avaliação física com todos os dados válidos
    Given que o aluno é cadastrado com a matrícula "902.142.720-60"
    ...
    Then o sistema em relação a avaliação fisica informa "Avaliação física salva com sucesso"
  ...
  Scenario: Registro de avaliação física com dados incompletos
    ...
    Then o sistema em relação a avaliação fisica informa "É necessário preencher todos os campos da avaliação física"
  ...
  Scenario: Consulta de histórico sem avaliações registradas
    Given que o aluno com a matrícula "159.621.460-00" ainda não possui avaliações físicas registradas
    When o aluno solicita o histórico de avaliações
    Then o sistema em relação a avaliação fisica informa "Esse aluno não tem avaliações registradas"
  ...
  Scenario: Aluno apresenta evolução corporal positiva
    ...
    Then o sistema em relação a evolução física informa "Você demonstrou evolução em: Massa muscular esquelética, Nível de gordura visceral e Tamanho de cintura. Parabéns!"
  ...
  Scenario: Aluno sem evolução ou com regressão corporal
    ...
    Then o sistema em relação a evolução física informa "Você não mostrou evolução nos seus treinos... É melhor revisar o seu treino com um professor"
