...
CREATE TABLE PLANO_TREINO (
    ID SERIAL PRIMARY KEY,
    PLT_PROFESSOR_ID INTEGER NOT NULL,
    DATA_CRIACAO DATE NOT NULL DEFAULT CURRENT_DATE,
    DATA_VALIDADE_SUGERIDA DATE,
    
    CONSTRAINT fk_plano_treino_professor FOREIGN KEY (PLT_PROFESSOR_ID) REFERENCES PROFESSOR(ID)
);

CREATE INDEX idx_plano_treino_professor ON PLANO_TREINO(PLT_PROFESSOR_ID);
COMMENT ON TABLE PLANO_TREINO IS 'Aggregate Root: PlanoDeTreino - Plano de exercícios personalizado';
...
CREATE TABLE TREINO_DIARIO (
    ...
    CONSTRAINT fk_treino_diario_plano FOREIGN KEY (PLT_ID) REFERENCES PLANO_TREINO(ID) ON DELETE CASCADE,
    CONSTRAINT chk_treino_letra CHECK (LETRA IN ('A', 'B', 'C', 'D', 'E', 'F', 'G')),
...
CREATE INDEX idx_treino_diario_plano ON TREINO_DIARIO(PLT_ID);
...
CREATE TABLE ITEM_EXERCICIO (
    ...
    CONSTRAINT fk_item_exercicio_treino FOREIGN KEY (TRD_ID) REFERENCES TREINO_DIARIO(ID) ON DELETE CASCADE,
    CONSTRAINT chk_exercicio_series CHECK (SERIES > 0)
);

CREATE INDEX idx_item_exercicio_treino ON ITEM_EXERCICIO(TRD_ID);
COMMENT ON TABLE ITEM_EXERCICIO IS 'Value Object: Exercício específico no treino diário';
COMMENT ON COLUMN ITEM_EXERCICIO.REPETICOES IS 'Exemplo: 8-12, 15, 30s, etc.';

-- Trigger para reordenação automática de treinos
CREATE OR REPLACE FUNCTION reordenar_treinos_apos_exclusao()
RETURNS TRIGGER AS $$
DECLARE
    treino_record RECORD;
    nova_letra VARCHAR(10);
    contador INTEGER := 0;
    letras VARCHAR[] := ARRAY['A', 'B', 'C', 'D', 'E', 'F', 'G'];
BEGIN
    FOR treino_record IN 
        SELECT id 
        FROM TREINO_DIARIO 
        WHERE PLT_ID = OLD.PLT_ID 
        ORDER BY LETRA
    LOOP
        contador := contador + 1;
        nova_letra := letras[contador];
        
        UPDATE TREINO_DIARIO 
        SET LETRA = nova_letra,
            POSICAO = contador - 1
        WHERE id = treino_record.id;
    END LOOP;
...
CREATE TRIGGER trigger_reordenar_treinos
    AFTER DELETE ON TREINO_DIARIO
    FOR EACH ROW
    EXECUTE FUNCTION reordenar_treinos_apos_exclusao();

COMMENT ON FUNCTION reordenar_treinos_apos_exclusao() IS 
    'Reordena automaticamente as letras dos treinos após a exclusão. Ex: ao excluir treino B, o treino C vira B, o D vira C, etc.';
...
CREATE TABLE ALUNO_PLANO_HISTORICO (
    ...
    CONSTRAINT fk_historico_plano FOREIGN KEY (PLANO_TREINO_ID) REFERENCES PLANO_TREINO(ID) ON DELETE CASCADE
);

ALTER TABLE ALUNO ADD CONSTRAINT fk_aluno_plano_ativo FOREIGN KEY (PLANO_ATIVO_ID) REFERENCES PLANO_TREINO(ID);
...
CREATE TABLE CHECKIN (
    ...
    CONSTRAINT fk_checkin_plano FOREIGN KEY (CONTEXTO_PLANO_TREINO_ID) REFERENCES PLANO_TREINO(ID),
